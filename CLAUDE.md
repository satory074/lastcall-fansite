...
| `judges` | `src/content/judges/<slug>.json` | name, age, area, store, sns, `thumbnailVideoId?` (the 14 fixed judges) |
...
- **No performer photos hosted directly.** 画像参照は **YouTube が配信するサムネイル (`i.ytimg.com/vi/<id>/hqdefault.jpg`)** に限定する。具体的には (a) **番組本編**のサムネ (`episodes.youtubeId`) を EpisodeCard / 詳細ページに使用、(b) **判事本人が自ら運営する YouTube チャンネルの動画**サムネ (`judges.thumbnailVideoId`) を PersonCard アバター / 判事プロフィールのヒーローに使用。番組以外の動画を使う場合はアップロード者＝本人で本人が同意して写っている前提のものに限る — 所属店・他社チャンネル経由の動画はグレーのため使わない。Instagram やプライベート写真の直接ホストは 著作権 + 肖像権 + パブリシティ権 を同時に侵害するため厳禁。SNS リンクは可。
