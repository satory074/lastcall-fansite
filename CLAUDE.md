...
`PersonCard` also takes an optional **`avatar`** prop — `{ initial: string }` — rendering a 48px circular **monogram badge** (name's first grapheme) at the card's top-left, styled with `var(--color-gold)`/`var(--color-gold-glow)` so it auto-recolors gold→platinum per `data-show`. On `/people/` (`PeopleView.astro`) **both** cinderella and queen cards pass `{ initial: Array.from(name)[0] ?? "?" }`. This gives performers a clean, uniform "profile icon" look **without hosting/hotlinking any photo** — a deliberate choice over cropping YouTube thumbnails (the show's thumbnails are busy title-card graphics that read as clutter at avatar size) and the only legally-safe option given the 著作権/肖像権/パブリシティ権 guardrail below. The badge is intentionally **not** `z-10`, so the card-wide `<a>::before` overlay stays on top and the badge is part of the card's click target; it carries no spoiler markup (`aria-hidden`).
